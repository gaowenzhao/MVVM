...
object Httper {
    ...
    private fun initOkHttpClient() {
        val interceptor =
            HttpLoggingInterceptor(HttpLoggingInterceptor.Logger { message -> /*Loger.show("=== $message", tag = "OkHttp")*/ }).setLevel(
...
//        Loger.show("no network", tag = mTag)
        ...
        val cache = Cache(
            File(BaseApplication.appContext.cacheDir, "HttpCache"),
            (cacheSize).toLong()
        )
        val builder =
            if (BuildConfig.DEBUG) OkHttpClient.Builder().proxy(Proxy.NO_PROXY) else OkHttpClient.Builder()
